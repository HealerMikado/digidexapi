...
def get_all_digimons(page_size: int = 10, page: int = 1,
    ...
    digimons = crud.get_digimons(page_size=page_size,
                                 page=page,
                                 xantibody=xantibody,
    ...
    pagination = utils.paginaton(
        path, count_digimons, page, page_size, len(digimons),
        xantibody=xantibody,
        id_field=id_field,
        id_level=id_level,
        id_type=id_type,
...
def get_digmon_by_name_or_id(id_or_name: Union[int, str], db: Session = Depends(get_db)):
    ...
    for digivolution in digimon.digivolve_to:
        digivolutions.append(
            schemas.Digivolution(
                name=digivolution.digimon_next.name,
                condition=digivolution.condition))
    pre_digivolutions = []
...
def get_all_levels(page_size: int = 5, page: int = 1, db: Session = Depends(get_db)):
    levels, count_levels = crud.get_levels(page_size=page_size,
                                           page=page,
                                           db=db)

    path = "/level"
    pagination = utils.paginaton(
        path, count_levels, page, page_size, len(levels))
    return schemas.Page(content=levels, pagination=pagination)


@app.get("/attribut", response_model=schemas.Page)
def get_all_attributes(page_size: int = 5, page: int = 1, db: Session = Depends(get_db)):
    attributes, count_attributes = crud.get_attributes(page_size=page_size,
                                                       page=page,
                                                       db=db)
...
def get_all_levels(page_size: int = 5, page: int = 1, db: Session = Depends(get_db)):
    fields, count_fields = crud.get_fields(page_size=page_size,
                                           page=page,
                                           db=db)

    path = "/field"
    pagination = utils.paginaton(
        path, count_fields, page, page_size, len(fields))
...
def get_all_types(page_size: int = 5, page: int = 1, db: Session = Depends(get_db)):
    types, count_types = crud.get_types(page_size=page_size,
                                        page=page,
                                        db=db)

    path = "/type"
    pagination = utils.paginaton(
        path, count_types, page, page_size, len(types))
    return schemas.Page(content=types, pagination=pagination)


@app.get("/skill", response_model=schemas.Page)
def get_all_skills(page_size: int = 10, page: int = 1,
                   name_contains: Union[str, None] = None,
                   description_contains: Union[str, None] = None,
                   db: Session = Depends(get_db)):
    skills, count_skills = crud.get_skills(page_size=page_size,
                                           page=page,
                                           name_contains=name_contains,
    ...
    path = "/skill"
    pagination = utils.paginaton(
        path, count_skills, page, page_size, len(skills))
