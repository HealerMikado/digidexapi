...
def get_digimons(page_size: int, page: int,
    ...
    if id_type is not None:
        query = query.filter(models.Digimon.types.any(
            models.Type.id.__eq__(id_type)
        ))
    if id_field is not None:
        query = query.filter(models.Digimon.fields.any(
            models.Field.id.__eq__(id_field)
        ))
    if id_level is not None:
        query = query.filter(models.Digimon.levels.any(
            models.Level.id.__eq__(id_level)
        ))
    if id_attribute is not None:
        query = query.filter(models.Digimon.attributes.any(
            models.Attribute.id.__eq__(id_attribute)
        ))
    if name_contains:
...
def get_digimon_by_name(digimon_name: str, db: Session):
    return db.query(models.Digimon) \
        ...
        .filter(models.Digimon.name.like(digimon_name)).one()
...
def get_digimon_by_id(digimon_id: int, db: Session):
    return db.query(models.Digimon) \
        ...
        .options(joinedload(models.Digimon.digivolved_from)) \
        .options(joinedload(models.Digimon.digivolve_to)) \
        .filter(models.Digimon.id.__eq__(digimon_id)).one()


def get_digivolution(db: Session, id_digimon):
    return db.query(models.Digimon) \
        .join(models.Digimon.digivolve_to) \
        .where(models.Digivolution.id_digimon_prior == id_digimon).all()
...
def get_skills(page_size: int, page: int,
               description_contains: Union[str, None],
               name_contains: Union[str, None], db: Session):
