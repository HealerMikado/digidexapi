...
class API(Construct):
    def __init__(self, scope: Construct, id_: str, *,
        ...
        db_user = ssm.StringParameter \
            .from_string_parameter_attributes(
            self,
            "digidex_api",
            parameter_name="/digimon-api/db/user"
        ).string_value
        digidex_lambda = python.PythonFunction(
            self, "Digidexapi_lambda",
            entry=str(pathlib.Path(__file__).parent.joinpath("runtime", "digidex-api").resolve()),  # required
        ...
        route53.ARecord(self, "Digidexapi_apigtwAlias",
